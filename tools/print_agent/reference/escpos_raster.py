...
def image_to_escpos_raster(
    ...
    *,
    ...
    img = _prepare_mono_image(data, max_width=max_width)
...
def build_logo_block(cfg: dict, logo_url: str | None, paper_width: int) -> bytes:
    if not logo_url:
        return b""
    try:
        ...
        if not raw:
            return b""
        raster = image_to_escpos_raster(raw, max_width=paper_width_dots(paper_width))
