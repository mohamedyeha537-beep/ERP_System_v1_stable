...
def accommodation_segment_for_room(
    ...
    *,
    ...
    if room is not None and room.nightly_price is not None:
        nights = (seg_out - seg_in).days
        return (Decimal(str(room.nightly_price)) * Decimal(nights)).quantize(Decimal("0.001"))
    rt_id = (room.room_type_id if room is not None else None) or room_type_id
    if rt_id:
        return accommodation_total(db, room_type_id=rt_id, check_in=seg_in, check_out=seg_out)
    return Decimal("0")
