"""تصنيفات فواتير الأدوات والاستهلاكات — يُدارها المدير."""
...
SETTING_KEY = "consumable_purchase_categories_json"

DEFAULT_CATEGORIES: list[dict] = [
    {"key": "tissues", "label": "مناديل وورق", "active": True},
    {"key": "cleaners", "label": "منظفات", "active": True},
    {"key": "office", "label": "أدوات مكتبية", "active": True},
    {"key": "kitchen", "label": "مستهلكات مطبخ", "active": True},
...
def load_consumable_categories(db: Session) -> list[dict]:
...
def save_consumable_categories(db: Session, items: list[dict]) -> None:
    clean: list[dict] = []
    seen: set[str] = set()
    for item in items:
...
def active_consumable_categories(db: Session) -> list[tuple[str, str]]:
    return [
        (c["key"], c["label"])
        for c in load_consumable_categories(db)
        if c.get("active", True)
    ]


def consumable_category_label(db: Session, category_key: str) -> str:
    key = (category_key or "").strip()
    for c in load_consumable_categories(db):
        if c["key"] == key:
            return c["label"]
    raise ValueError("تصنيف الاستهلاك غير صالح.")
...
def add_category(db: Session, *, label: str) -> dict:
    ...
    items = load_consumable_categories(db)
    base = _slug(label)
    key = base
    n = 2
    ...
    items.append(row)
    save_consumable_categories(db, items)
    return row
...
def delete_category(db: Session, *, key: str) -> None:
    key = (key or "").strip()
    items = [c for c in load_consumable_categories(db) if c["key"] != key]
    if not items:
