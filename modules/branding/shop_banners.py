"""بانرات إعلانية لصفحة المتجر /shop."""
...
_SETTING_JSON = "brand_shop_banners_json"
_SETTING_INTERVAL = "brand_shop_banner_interval_seconds"
...
class ShopBannerError(Exception):
    pass
...
def get_shop_banners_admin(db: Session) -> dict[str, Any]:
    items = []
    ...
    return {
        "interval_seconds": banner_interval_seconds(db),
        "banners": items,
    }


def get_shop_banners_public(db: Session) -> dict[str, Any]:
    items = []
    for row in _load_raw(db):
...
def add_shop_banner(
    ...
    *,
    ...
    if len(items) >= _MAX_BANNERS:
        raise ShopBannerError(f"الحد الأقصى {_MAX_BANNERS} بانرات.")
    if not upload.filename:
        raise ShopBannerError("لم يُحدَّد ملف.")
    content = upload.file.read()
    if not content:
        raise ShopBannerError("الملف فارغ.")
    if len(content) > _MAX_BYTES:
        raise ShopBannerError("حجم الصورة يتجاوز 2 م.ب.")
    ext = Path(upload.filename).suffix.lower()
    if ext not in _ALLOWED_EXT:
        raise ShopBannerError("نوع الصورة غير مدعوم (jpg, png, webp).")
    fname = f"shop-banner-{uuid.uuid4().hex[:12]}{ext}"
...
def delete_shop_banner(db: Session, banner_id: str) -> bool:
